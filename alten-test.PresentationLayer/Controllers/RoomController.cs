...
namespace alten_test.PresentationLayer.Controllers
{
    ...
    public class RoomController : ControllerBase
    {
        ...
        [HttpGet("Available")]
        [Produces("application/json")]
        public async Task<ActionResult<PaginationResultDto<RoomDto>>> GetAvailableRooms(
            [FromQuery] int pageNumber,
            [FromQuery] int pageSize,
            [FromQuery] string filterBy,
            [FromQuery] string searchTerm,
        ...
        )
        {
            ...
            switch (filterBy)
            {
                case "type":
                    filterProperty = nameof(RoomDto.Type);
                    if (!Enum.IsDefined(typeof(RoomType), searchTerm))
                    {
                        return BadRequest();
                    }
                    break;
            ...
            var availableRooms = await _roomService.GetAvailable(pageInfo, startDate, endDate);

            if (availableRooms.Data == null)
            {
                return NotFound();
            }
